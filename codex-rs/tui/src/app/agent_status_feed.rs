...
fn activity_summary(item: &ThreadItem) -> Option<String> {
    let summary = match item {
        ...
        ThreadItem::FileChange { changes, .. } => {
            return bounded_summary(&format!("更新了 {} 个文件", changes.len()));
        }
        ...
        ThreadItem::CollabAgentToolCall { tool, .. } => {
            let action = match tool {
                ...
                CollabAgentTool::SpawnAgent => "已生成一个代理",
                CollabAgentTool::SendInput => "已向代理发送输入",
                CollabAgentTool::ResumeAgent => "已恢复代理",
                CollabAgentTool::Wait => "已等待代理",
                CollabAgentTool::CloseAgent => "已关闭代理",
            };
        ...
        ThreadItem::SubAgentActivity {
            ...
            let action = match kind {
                ...
                SubAgentActivityKind::Completed => "已完成",
            };
