...
impl App {
    pub(super) async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        build_config_on_runtime_worker(builder, format!("为 cwd {cwd_display} 重新构建配置失败"))
            .await
    }
    ...
    pub(super) async fn rebuild_config_for_permission_profile(
        ...
        build_config_on_runtime_worker(
            builder,
            format!("为权限配置文件 {profile_id} 重新构建配置失败"),
        )
        .await
    }
    ...
    pub(super) async fn apply_permission_profile_selection(
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                self.chat_widget
                    .add_error_message(format!("设置权限配置文件 `{profile_id}` 失败：{err}"));
                return false;
            }
        };
    ...
    pub(super) async fn select_permission_profile(
        ...
        let Some(thread_id) = self.chat_widget.thread_id() else {
            ...
            self.chat_widget
                .add_error_message("请等待任务连接后再选择权限。".into());
            return;
        };
            ...
            && self.chat_widget.is_user_turn_pending_or_running()
        {
            self.chat_widget
                .retain_input_after_failed_permission_selection();
            self.chat_widget
                .add_error_message("请等待当前回合结束后再更改权限。".into());
            return;
        }
        ...
        match app_server.thread_settings_update(params).await {
            Ok(true) => {
                ...
                self.chat_widget.add_info_message(
                    format!("已请求选择权限：{}", selection.display_label),
                    /*hint*/ None,
                );
                self.chat_widget.submit_initial_user_message_if_pending();
            }
            Ok(false) => {
                self.chat_widget
                    .retain_input_after_failed_permission_selection();
                self.chat_widget
                    .add_error_message("选择权限需要更新版本的 app server。".into());
            }
            Err(error) => {
                self.chat_widget
                    .retain_input_after_failed_permission_selection();
                self.chat_widget
                    .add_error_message(format!("选择权限失败：{error:#}"));
            }
        }
    }
    ...
    pub(super) fn reject_pending_permission_change(&mut self) -> bool {
        ...
        {
            self.chat_widget
                .add_error_message("请等待权限更新完成后再更改权限。".into());
            return true;
        }
        false
    }
    ...
    pub(super) fn reject_pending_permission_root_switch(&mut self) -> bool {
        ...
        self.chat_widget
            .add_error_message("请等待权限更新完成后再切换任务。".into());
        true
    }
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "已保存{setting}，但 Codex 无法刷新有效配置：{err}"
                ));
                None
            }
        }
    ...
    pub(super) fn apply_runtime_policy_overrides(
                ...
                .set(policy.policy().to_core())
        {
            ...
            self.chat_widget
                .add_error_message(format!("沿用审批策略覆盖值失败：{err}"));
        }
                    ...
                    == RuntimePermissionProfileTurnOverride::LegacySandbox)
        {
                ...
                .can_set(&profile_override.approvals_reviewer)
            {
                ...
                Err(error) => self
                    .chat_widget
                    .add_error_message(format!("沿用审批审查者失败：{error}")),
            }
            match config
                .permissions
                .set_permission_profile_from_session_snapshot(
                ...
                Err(err) => {
                    ...
                    self.chat_widget
                        .add_error_message(format!("沿用权限配置文件覆盖值失败：{err}"));
                }
            }
        }
    }
    ...
    pub(super) fn try_set_builtin_active_permission_profile_on_config(
        ...
        let Some(permission_profile) =
            ...
            self.chat_widget.add_error_message(format!(
                "{user_message_prefix}：不支持活动权限配置文件 `{}`",
    ...
    pub(super) async fn update_feature_flags(
            ...
            if feature == Feature::GuardianApproval {
                ...
                if effective_enabled {
                    ...
                    if previous_approvals_reviewer != auto_review_preset.approvals_reviewer {
                        permissions_history_label = Some("为我审批");
                    }
        ...
        if write_response.status == WriteStatus::OkOverridden {
            ...
            self.chat_widget
                .add_error_message(format!("实验性功能更改已保存但未应用：{message}"));
                ...
                )
        {
            ...
            self.chat_widget
                .add_error_message(format!("启用“为我审批”失败：{err}"));
        }
    ...
    pub(super) async fn update_memory_settings(
        ...
        if write_response.status == WriteStatus::OkOverridden {
            ...
            self.app_event_tx.send(AppEvent::FollowTranscript);
            self.chat_widget
                .add_error_message(format!("记忆设置更改已保存但未应用：{message}"));
    ...
    pub(super) fn reject_remote_resume_permission_override(&mut self, config: &Config) -> bool {
        ...
        if explicitly_selected {
            self.add_agents_overview_error("恢复远程任务时不支持权限覆盖。".into());
            return true;
        }
        false
    }
    ...
    fn sync_feature_state_from_effective_config(
        ...
        if let Some(policy) = approval_policy_from_effective_config(effective_config) {
                ...
                .set(policy.to_core())
            {
                ...
                self.chat_widget
                    .add_error_message(format!("刷新已覆盖的“为我审批”设置失败：{err}"));
            } else {
                self.chat_widget.set_approval_policy(policy);
            }
        }
    ...
    pub(super) async fn verify_windows_sandbox_mode_after_setup(
        ...
        self.chat_widget
            .add_error_message("Windows 沙箱设置已完成，但其模式被有效配置覆盖。".to_string());
        false
    }
}
