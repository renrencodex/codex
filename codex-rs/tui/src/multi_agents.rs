...
fn sub_agent_activity_title(kind: SubAgentActivityKind, agent_path: &str) -> Line<'static> {
    let (prefix, path) = match kind {
        ...
        SubAgentActivityKind::Completed => ("已完成 ", agent_path),
    };
...
fn waiting_begin(
    ...
    let title = match receiver_agents.as_slice() {
        ...
        [] => title_text("正在等待代理"),
        _ => title_text(format!("正在等待 {} 个代理", receiver_agents.len())),
    };
...
fn agent_label_spans(agent: AgentLabel<'_>) -> Vec<Span<'static>> {
    ...
    if let Some(nickname) = nickname {
        ...
        spans.push(Span::from("代理").fg(accent_color()));
    }
...
fn wait_complete_lines(
    ...
    if entries.is_empty() {
        vec![Line::from(Span::from("尚无代理完成任务"))]
    } else {
        entries
            .into_iter()
...
fn status_summary_spans(status: &CollabAgentState) -> Vec<Span<'static>> {
    match status.status {
        CollabAgentStatus::PendingInit => vec![Span::from("等待初始化").fg(accent_color())],
        CollabAgentStatus::Running => vec![Span::from("运行中").fg(accent_color()).bold()],
        // Allow `.yellow()`
        ...
        CollabAgentStatus::Errored => {
            error_summary_spans(status.message.as_deref().unwrap_or("代理出错"))
        }
