...
impl AnalyticsView {
    pub(super) fn footer_lines(&self, width: usize, scrollable: bool) -> Vec<Line<'static>> {
        ...
        if self.show_help {
            actions.push((self.hint(ListAction::Cancel), "返回"));
        } else if !self.zoomed {
            actions.push((self.hint(ListAction::Accept), "聚焦"));
            actions.push(("tab".into(), "卡片"));
        } else {
            match self.section {
                Section::Summary => {}
                Section::Plan => {
                    actions.push((pair(ListAction::MoveUp, ListAction::MoveDown), "周期"));
                }
                Section::Chats => {
                    actions.push((pair(ListAction::MoveUp, ListAction::MoveDown), "行"));
                }
                Section::Usage
                | Section::Credits
                | Section::Activity
                | Section::Plugins
                | Section::Skills => {
                    actions.push((pair(ListAction::MoveLeft, ListAction::MoveRight), "日期"));
                }
            }
            ...
            {
                actions.push((self.hint(ListAction::Accept), "详情"));
            }
        }
        if scrollable {
            actions.push((pair(ListAction::PageUp, ListAction::PageDown), "滚动"));
        }
        if !self.show_help {
            actions.push((self.hint(ListAction::Cancel), "返回"));
        }
