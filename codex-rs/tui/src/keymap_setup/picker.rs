...
const KEYMAP_CONTEXT_TABS: &[KeymapContextTab] = &[
    ...
    KeymapContextTab {
        ...
        label: "输入框",
        description: "输入框的提交和排队快捷键。",
        contexts: &["composer"],
    },
    KeymapContextTab {
        id: "editor-shortcuts",
        label: "编辑器",
        description: "行内编辑器的移动和编辑快捷键。",
        contexts: &["editor"],
    },
    KeymapContextTab {
        id: "vim-shortcuts",
...
fn build_keymap_picker_params_for_action(
    ...
    for tab in KEYMAP_CONTEXT_TABS {
        ...
        tabs.push(SelectionTab {
            ...
            items: keymap_selection_items(tab_rows, "此组中没有快捷键", "此组中没有可配置的操作。"),
        });
    }
...
fn keymap_debug_tab() -> SelectionTab {
    SelectionTab {
        ...
        items: vec![SelectionItem {
            name: "检查按键".to_string(),
            description: Some("按 Enter 开始，然后按任意键进行检查；Ctrl+C 退出。".to_string()),
...
fn keymap_header(description: String, summary: String) -> Box<dyn Renderable> {
    ...
    header.push(Line::from("按键映射".bold()));
