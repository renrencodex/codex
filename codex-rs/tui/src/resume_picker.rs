...
fn search_line(state: &PickerState, width: u16) -> Line<'_> {
    ...
    if state.query.is_empty() {
        "输入以搜索".set_style(secondary_text_style()).into()
    } else {
        truncate_text(&format!("搜索：{}", state.query), usize::from(width)).into()
    }
}
...
fn toolbar_line(state: &PickerState, compact: bool) -> Line<'static> {
    ...
    if matches!(state.action, SessionPickerAction::Resume) {
        ...
        if compact {
            ...
            spans.push("状态：".set_style(secondary_text_style()));
            spans.push(toolbar_value(
                "活跃",
...
fn sort_control_spans(state: &PickerState, compact: bool) -> Vec<Span<'static>> {
    ...
    if compact {
        return vec![
            "排序：".set_style(secondary_text_style()),
            toolbar_value(
...
fn filter_control_spans(state: &PickerState, compact: bool) -> Vec<Span<'static>> {
    ...
    if compact || state.filter_cwd.is_none() {
        return vec![
            "筛选：".set_style(secondary_text_style()),
            toolbar_value(
...
fn footer_hint_lines(state: &PickerState, width: u16) -> Vec<Line<'static>> {
    if state.is_transcript_loading() {
        let hints = [
            PickerFooterHint {
                ...
                compact_label: String::from("对话记录"),
                priority: 0,
            },
            PickerFooterHint {
    ...
    let (esc_label, esc_compact_label) = if state.query.is_empty() {
        match state.launch_context {
            SessionPickerLaunchContext::Startup => ("开始新会话", "新会话"),
...
fn render_expanded_session_details(
    ...
    let branch = format!(
        ...
        row.git_branch.as_deref().unwrap_or("无分支")
    );
    ...
    vec![
        ...
        vec![
            "  │ ".set_style(secondary_text_style()),
            "对话：".set_style(secondary_text_style()),
        ]
        .into(),
    ]
}
...
fn format_relative_time_long(reference: DateTime<Utc>, ts: DateTime<Utc>) -> String {
    ...
    if seconds < 60 {
        return plural_time(seconds, "秒");
    }
    let minutes = seconds / 60;
    if minutes < 60 {
        return plural_time(minutes, "分钟");
    }
    let hours = minutes / 60;
    if hours < 24 {
        return plural_time(hours, "小时");
    }
    plural_time(hours / 24, "天")
}
...
fn render_empty_state_line(state: &PickerState) -> Line<'static> {
    if !state.query.is_empty() {
        ...
        {
            return vec!["正在搜索…".set_style(secondary_text_style()).italic()].into();
        }
    ...
    if state.pagination.is_loading() {
        if state.all_rows.is_empty() && state.pagination.num_scanned_files == 0 {
            return vec!["正在加载会话…".set_style(secondary_text_style()).italic()].into();
        }
        return vec![
            "正在加载更早的会话…"
                .set_style(secondary_text_style())
    ...
    vec!["还没有会话".set_style(secondary_text_style()).italic()].into()
}
...
mod tests {
    ...
    fn relative_time_formats_zero_seconds_as_now() {
        ...
        assert_eq!(format_relative_time(reference, Some(reference)), "刚刚");
        assert_eq!(
    ...
    fn long_relative_time_uses_words() {
        ...
        assert_eq!(format_relative_time_long(reference, reference), "刚刚");
        assert_eq!(
    ...
    fn expanded_session_details_include_metadata() {
        ...
        assert!(rendered.contains(
            "会话：    feat(tui): add raw scrollback mode (019dabc1-0ef5-7431-b81c-03037f51f62c)"
        ));
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains(" 无分支"));
        assert_metadata_order(&footer[0], "⌁ /tmp/codex", " 无分支");
    }
    ...
    fn hint_line_switches_esc_label_for_search_mode() {
        ...
        assert!(footer_lines_text(&state, /*width*/ 220).contains("esc 开始新会话"));
    ...
    fn hint_line_switches_density_label() {
        ...
        assert!(footer_lines_text(&state, /*width*/ 220).contains("ctrl+o 紧凑视图"));
        ...
        assert!(!footer_lines_text(&state, /*width*/ 220).contains("更改选项"));
    ...
    fn hint_line_compacts_on_narrow_width() {
        ...
        assert!(rendered.contains("esc 新会话"));
        ...
        assert!(!rendered.contains("聚焦排序/筛选"));
    }
    ...
    fn hint_line_shows_loading_transcript_mode() {
        ...
        assert!(rendered.contains("正在加载 对话记录"));
    ...
    async fn loaded_transcript_waits_for_loading_frame_before_opening_overlay() {
        ...
        .unwrap();
        state.pending_transcript_open = Some(thread_id);
        let cells: TranscriptCells = vec![Arc::new(PlainHistoryCell::new(vec!["对话记录".into()]))];
...
"#
    ...
    async fn enter_on_row_without_resolvable_thread_id_shows_inline_error() {
        ...
        assert_eq!(
            state.inline_error,
            Some(String::from("无法从 /tmp/missing.jsonl 读取会话元数据"))
        );
    }
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        assert!(rendered.contains("建议的计划"));
