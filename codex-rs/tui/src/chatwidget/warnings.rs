...
impl super::ChatWidget {
    ...
    /// provider the fallback-metadata warning fires for every model and leaves
    /// nothing to act on. Checked on both the startup and the runtime path.
    pub(super) fn is_unactionable_warning(&self, message: &str) -> bool {
        self.config.model_provider_id != codex_model_provider_info::OPENAI_PROVIDER_ID
            && fallback_model_metadata_warning_slug(message).is_some()
    }
