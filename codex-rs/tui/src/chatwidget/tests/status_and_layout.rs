...
async fn model_metadata_warning_is_hidden_for_third_party_providers() {
    for startup_complete in [false, true] {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
        chat.config.model_provider_id = "my-proxy".to_string();
        chat.warning_display_state.startup_complete = startup_complete;
        ...
        assert_eq!(
            cells.len(),
            1,
            "expected only the generic warning (startup_complete = {startup_complete})"
        );
