...
impl ChatWidget {
    ...
    pub(crate) fn open_marketplace_add_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            "添加插件市场".to_string(),
            "owner/repo、Git URL 或本地插件市场路径".to_string(),
            String::new(),
        ...
        match result {
            Ok(response) => {
                ...
                let message = if response.already_added {
                    format!("插件市场 {} 已添加。", response.marketplace_name)
                } else {
                    format!("已添加插件市场 {}。", response.marketplace_name)
                };
                self.add_info_message(
                    message,
                    Some(format!(
    ...
    pub(crate) fn on_marketplace_remove_loaded(
        ...
        match result {
            Ok(response) => {
                self.plugins_active_tab_id = Some(ALL_PLUGINS_TAB_ID.to_string());
                self.add_info_message(
                    format!("已移除插件市场 {marketplace_display_name}。"),
                    Some(match response.installed_root {
                        Some(installed_root) => {
                            format!("插件市场根目录：{}", installed_root.as_path().display())
                        }
                        None => format!("已移除 {} 的插件市场配置。", response.marketplace_name),
                    }),
                );
            }
            Err(_) => {
    ...
    pub(crate) fn on_marketplace_upgrade_loaded(
        ...
        match result {
            Ok(response) => {
                ...
                if selected_count == 0 {
                    self.add_info_message(
                        "没有可升级的已配置 Git 插件市场。".to_string(),
                        Some("只能升级已配置的 Git 插件市场。".to_string()),
                    );
                    return;
                }
                ...
                if upgraded_count == 0 && error_count == 0 {
                    let message = if selected_count == 1 {
                        ...
                        format!("已检查 {selected_count} 个插件市场；均为最新版本。")
                    };
                    self.add_info_message(
                        message,
                        Some(format!(
                ...
                if upgraded_count > 0 {
                    self.add_info_message(
                        format!("已升级 {upgraded_count} 个插件市场。"),
                        Some(format!(
                            "已更新根目录：{}",
                            response
                                .upgraded_roots
    ...
    fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
        ...
        if is_installed {
            ...
            items.push(SelectionItem {
                name: "我已安装".to_string(),
                description: Some("确认后继续设置下一个应用。".to_string()),
        ...
        items.push(SelectionItem {
            ...
            selected_description: Some("放弃设置其余必需应用。".to_string()),
            actions: vec![Box::new(|tx| {
    ...
    fn finish_plugin_install_auth_flow(&mut self, abandoned: bool) {
        ...
        if abandoned {
            self.add_info_message(
                format!("已跳过插件 {} 的剩余应用设置。", flow.plugin_display_name),
                Some("安装所需应用之前，此插件可能无法使用。".to_string()),
            );
        } else {
            self.add_info_message(
