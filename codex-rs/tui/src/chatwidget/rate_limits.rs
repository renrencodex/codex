        ...
        if let Some(secondary_used_percent) = secondary_used_percent {
            ...
            if let Some(threshold) = highest_secondary {
                ...
                warnings.push(format!(
                    "请注意，你的{limit_label}限额剩余不足 {remaining_percent:.0}%。运行 /status 查看详情。"
                ));
            }
        }
...
impl ChatWidget {
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let items = vec![
            ...
            SelectionItem {
                ...
                description: Some("以后不再显示有关切换模型的速率限制提醒。".to_string()),
                selected_description: None,
                is_current: false,
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            title: Some("即将达到速率限制".to_string()),
            subtitle: Some(format!("是否切换到 {switch_model} 以减少点数用量？")),
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            AddCreditsNudgeCreditType::Credits => (
                "已达到工作区点数限制",
                "你的工作区点数已用完。请让工作区所有者添加点数。是否通知所有者？",
            ),
            AddCreditsNudgeCreditType::UsageLimit => (
                "已达到用量限制",
                "请向所有者申请提高限制，以继续使用 Codex。是否申请？",
            ),
        };
    ...
    pub(crate) fn finish_add_credits_nudge_email_request(
        ...
        let message = match (credit_type, result) {
            ...
            (AddCreditsNudgeCreditType::UsageLimit, Err(_)) => "无法请求提高限额，请重试。",
        };
