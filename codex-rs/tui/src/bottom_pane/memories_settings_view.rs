...
impl MemoriesSettingsView {
    ...
    fn settings_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Paragraph::new(Line::from("记忆".bold())).wrap(Wrap { trim: false }));
        header.push(
    ...
    fn reset_confirmation_header(&self) -> ColumnRenderable<'_> {
        let mut header = ColumnRenderable::new();
        header.push(Paragraph::new(Line::from("重置所有记忆？".bold())).wrap(Wrap { trim: false }));
        header.push(
            Paragraph::new(Line::from(
                "这将清除当前 Codex 主目录中的本地记忆文件和 rollout 摘要。".dim(),
            ))
            .wrap(Wrap { trim: false }),
        );
        header
