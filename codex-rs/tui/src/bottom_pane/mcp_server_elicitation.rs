...
const ANSWER_PLACEHOLDER: &str = "输入回答";
const OPTIONAL_ANSWER_PLACEHOLDER: &str = "输入回答（可选）";
    ...
    fn footer_tips(&self) -> Vec<Line<'static>> {
        ...
        if let Some(submit_hint) = submit_hint.map(ShortcutHint::display_label) {
            if self.field_count() == 1 {
                tips.push(shortcut(&submit_hint, "提交"));
            } else if is_last_field {
                tips.push(shortcut(&submit_hint, "全部提交"));
            } else {
                tips.push(shortcut(&submit_hint, "提交回答"));
            }
        }
        if self.field_count() > 1 {
            if self.current_field_is_select() {
