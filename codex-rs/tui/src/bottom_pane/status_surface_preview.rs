...
impl StatusSurfacePreviewItem {
    fn placeholder(self) -> &'static str {
        match self {
            ...
            StatusSurfacePreviewItem::ThreadName => "会话名称",
            ...
            StatusSurfacePreviewItem::FiveHourLimit => "主要 0%",
            StatusSurfacePreviewItem::WeeklyLimit => "次要 0%",
            StatusSurfacePreviewItem::CodexVersion => "0.0.0",
            StatusSurfacePreviewItem::ContextWindowSize => "0 窗口",
            ...
            StatusSurfacePreviewItem::FastMode => "快速模式开启",
            StatusSurfacePreviewItem::RawOutput => "原始输出",
            StatusSurfacePreviewItem::WorkspaceHeadline => "工作区标题",
            StatusSurfacePreviewItem::Model => "gpt-5.2-codex",
            StatusSurfacePreviewItem::ModelWithReasoning => "gpt-5.2-codex 中等",
            StatusSurfacePreviewItem::Reasoning => "中等",
...
fn rate_limit_preview_copy(value: &str) -> Option<RateLimitPreviewCopy> {
    ...
    if value.starts_with("secondary usage ") {
        Some(RateLimitPreviewCopy {
            ...
            description: "次要用量限制的剩余额度（不可用时省略）",
        })
    } else if value.starts_with("usage ") {
        Some(RateLimitPreviewCopy {
            name: "usage-limit",
            description: "主要用量限制的剩余额度（不可用时省略）",
        })
    } else if value.starts_with("5h ") {
        Some(RateLimitPreviewCopy {
            name: "five-hour-limit",
            description: "5 小时用量限制的剩余额度（不可用时省略）",
        })
    } else if value.starts_with("daily ") {
        Some(RateLimitPreviewCopy {
            name: "daily-limit",
            description: "每日用量限制的剩余额度（不可用时省略）",
        })
    } else if value.starts_with("weekly ") {
        Some(RateLimitPreviewCopy {
            name: "weekly-limit",
            description: "每周用量限制的剩余额度（不可用时省略）",
        })
    } else if value.starts_with("monthly ") {
        Some(RateLimitPreviewCopy {
            name: "monthly-limit",
            description: "每月用量限制的剩余额度（不可用时省略）",
        })
    } else if value.starts_with("annual ") {
        Some(RateLimitPreviewCopy {
            name: "annual-limit",
            description: "每年用量限制的剩余额度（不可用时省略）",
        })
    } else {
        None
    }
