...
const OTHER_OPTION_DESCRIPTION: &str = "可以在备注中补充详细信息（tab）。";
const UNANSWERED_CONFIRM_TITLE: &str = "仍有未回答的问题，是否提交？";
    ...
    fn footer_tips(&self) -> Vec<Line<'static>> {
        ...
        if let Some(submit_key) = submit_key {
            let submit_tip = if question_count == 1 {
                shortcut(&submit_key, "提交回答")
            } else if is_last_question {
                shortcut(&submit_key, "全部提交")
            } else {
                shortcut(&submit_key, "提交回答")
            };
        ...
        if question_count > 1 {
            if self.has_options() && !self.focus_is_notes() {
                tips.push(shortcut("←/→", "切换问题"));
            } else if !self.has_options() {
                tips.push(shortcut("ctrl+p / ctrl+n", "切换问题"));
            }
        }
        ...
        {
            tips.push(shortcut(&interrupt_key.display_label(), "中断"));
        }
        tips
    }
...
mod tests {
    ...
    fn options_notes_focus_hides_question_navigation_tip() {
        ...
        assert_eq!(
            tip_texts,
            vec!["tab 添加备注", "enter 提交回答", "←/→ 切换问题", "esc 中断",]
        );
    ...
    fn freeform_shows_ctrl_p_and_ctrl_n_question_navigation_tip() {
        ...
        assert_eq!(
            tip_texts,
            vec!["enter 全部提交", "ctrl+p / ctrl+n 切换问题", "esc 中断",]
        );
    }
    ...
    fn freeform_footer_shows_configured_submit_binding() {
        ...
        let tips = overlay.footer_tips();
        let tip_texts = tips.iter().map(ToString::to_string).collect::<Vec<_>>();
        assert_eq!(tip_texts, vec!["ctrl+j 提交回答", "esc 中断"]);
    }
    ...
    fn request_user_input_uses_remapped_interrupt_binding_while_notes_are_visible() {
        ...
        assert_eq!(
            tip_texts,
            vec!["tab 或 esc 清除备注", "enter 提交回答", "f12 中断",]
        );
