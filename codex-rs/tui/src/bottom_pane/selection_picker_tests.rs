...
fn short_browser_keeps_header_tabs_search_and_footer() {
    ...
    for (width, height) in [(48, 6), (48, 7), (48, 13), (24, 6), (24, 7)] {
        ...
        assert!(text.contains("按键映射"));
        assert!(text.contains("常用"));
        assert!(text.contains("输入以搜索"));
