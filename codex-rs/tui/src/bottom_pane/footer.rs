    ...
    match state.hint {
        ...
        SummaryHintKind::Shortcuts => {
            if let Some(key) = key_hints.agents {
                ...
                line.push_span(" 查看智能体".set_style(secondary_text_style()));
            ...
            if let Some(key) = key_hints.toggle_shortcuts {
                line.extend(key.spans());
                line.push_span(" 查看快捷键".set_style(secondary_text_style()));
            }
        }
        SummaryHintKind::QueueMessage => {
            if let Some(key) = key_hints.queue {
                line.extend(key.spans());
                line.push_span(" 将消息加入队列".set_style(secondary_text_style()));
            }
        }
        SummaryHintKind::QueueShort => {
            if let Some(key) = key_hints.queue {
                line.extend(key.spans());
                line.push_span(" 加入队列".set_style(secondary_text_style()));
            }
        }
    };
...
pub(crate) fn side_conversation_context_line(label: &str) -> Line<'static> {
    ...
    let rest = if let Some(rest) = label.strip_prefix("Side ") {
        line.extend(["旁路".magenta().bold(), " ".into()]);
        rest
    } else {
        label
    };
    for (index, part) in rest.split(" · ").enumerate() {
        ...
        if let Some((keys, action)) = [" to switch", " 关闭", " for side"]
            .into_iter()
...
fn footer_from_props_lines(
    ...
    match props.mode {
        ...
        FooterMode::HistorySearch => {
            vec![Line::from("反向搜索历史：").set_style(secondary_text_style())]
        }
        FooterMode::ComposerEmpty => {
...
fn quit_shortcut_reminder_line(key: KeyBinding) -> Line<'static> {
    ...
    line.push_span(" 再按一次退出".set_style(secondary_text_style()));
    line
}
...
fn esc_hint_line(esc_backtrack_hint: bool) -> Line<'static> {
    ...
    if esc_backtrack_hint {
        ...
        line.push_span(" 再按一次编辑上一条消息".set_style(secondary_text_style()));
        line
    } else {
        let mut line = Line::from(esc.spans());
        line.push_span(" ".set_style(secondary_text_style()));
        line.extend(esc.spans());
        line.push_span(" 编辑上一条消息".set_style(secondary_text_style()));
        line
    }
}
...
pub(crate) fn context_window_line(percent: Option<i64>, used_tokens: Option<i64>) -> Line<'static> {
    if let Some(percent) = percent {
        ...
        return Line::from(vec![
            Span::from(format!("上下文剩余 {percent}%")).set_style(secondary_text_style()),
        ]);
    }
    ...
    if let Some(tokens) = used_tokens {
        ...
        return Line::from(vec![
            Span::from(format!("已使用 {used_fmt}")).set_style(secondary_text_style()),
        ]);
    }

    Line::from(vec![
        Span::from("上下文剩余 100%").set_style(secondary_text_style()),
    ])
}
...
mod tests {
    ...
    fn footer_status_line_truncates_to_keep_mode_indicator() {
        ...
        assert!(
            !collapsed.contains("按 shift+tab 切换"),
