...
const MACOS_APP_TOOLTIP: &str = "运行 `codex app` 打开桌面应用（需要时会在 macOS 上安装）。";
const LINUX_APP_TOOLTIP: &str = "在 Linux 上试试**桌面应用**：从 https://learn.chatgpt.com/docs/linux/linux-app 安装，然后运行 'chatgpt'。";
const FAST_TOOLTIP: &str = "*新功能* 使用 **/fast** 启用最快推理，但会增加方案用量。";
...
const FREE_GO_TOOLTIP: &str = "*新功能* 限时免费：你的方案已包含 Codex，一起构建吧。";
